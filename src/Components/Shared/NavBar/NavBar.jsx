...
const NavBar = () => {
  ...
  const normalLink =
    "text-gray-700 dark:text-gray-200 hover:text-[#35AC86] font-bold transition";

  return (
    <nav className="bg-white dark:bg-gray-900 shadow-md sticky top-0 z-[100] transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 md:px-5 flex justify-between items-center h-20">
        {/* Logo */}
        <Link to="/" className="group flex items-center gap-2">
          ...
          <div className="flex flex-col leading-none">
            <span className="text-lg font-black tracking-tighter dark:text-white uppercase">
              Scholarship
            </span>
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="dropdown dropdown-end">
                <div tabIndex={0} className="avatar cursor-pointer">
                  <div className="w-10 rounded-full ring-2 ring-[#35AC86] ring-offset-2 dark:ring-offset-gray-900">
                    <img
                      src={
                        user.photoURL ||
                        "https://i.ibb.co/mJR9n1C/default-avatar.png"
                      }
                      alt="User"
                    />
                  </div>
                ...
                <ul
                  tabIndex={0}
                  className="dropdown-content menu p-3 shadow-xl bg-white dark:bg-gray-800 rounded-xl w-52 border dark:border-gray-700 mt-4"
                >
                  <li>
                    <NavLink
                      to="/dashboard"
              ...
              <div className="flex gap-3">
                <NavLink
                  to="/auth/login"
                  className="px-5 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:text-gray-200 font-bold hover:bg-gray-50 dark:hover:bg-gray-800 transition"
                >
                  Login
                </NavLink>
                <NavLink
                  to="/auth/register"
                  className="px-5 py-2 rounded-lg bg-[#35AC86] text-white font-bold hover:opacity-90 transition"
                >
                  Register
                </NavLink>
              </div>
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 bg-white dark:bg-gray-900 border-t dark:border-gray-800 ${
      ...
      >
        <div className="flex flex-col px-6 gap-4">
          <NavLink
            ...
            className={({ isActive }) =>
              `text-lg font-bold ${
                isActive ? "text-[#35AC86]" : "dark:text-gray-200"
              }`
            }
          >
            Home
          ...
          <div className="border-t dark:border-gray-800 pt-4 mt-2">
            {user ? (
              <button
                onClick={handleLogout}
                className="w-full py-3 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 font-bold"
              >
                Logout
              </button>
            ) : (
              <div className="flex flex-col gap-3">
                <Link
                  onClick={() => setMenuOpen(false)}
                  to="/auth/login"
                  className="w-full py-3 text-center rounded-lg border dark:border-gray-700 dark:text-white font-bold"
                >
                  Login
                </Link>
                <Link
                  onClick={() => setMenuOpen(false)}
                  to="/auth/register"
                  className="w-full py-3 text-center rounded-lg bg-[#35AC86] text-white font-bold"
                >
                  Register
                </Link>
              </div>
